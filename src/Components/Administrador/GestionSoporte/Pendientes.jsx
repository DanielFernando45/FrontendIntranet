...
const Pendientes = () => {
    ...
    const fetchTickets = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_PORT_ENV}/soporte/espera`
        );
  ...
  const confirmResolution = async () => {
    try {
      const response = await fetch(
        ...
        }/soporte/finish/${currentTicketId}`,
        {
          method: "PATCH",
        }
      );
      ...
      if (!response.ok) {
        throw new Error(
          (await response.text()) || "Error al marcar como resuelto"
        );
      }

      // Actualizar lista de tickets eliminando el resuelto
      setTickets(
        tickets.filter((ticket) => ticket.id_soporte !== currentTicketId)
      );
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <>
      <div className="flex flex-col">
        ...
        {tickets.map((ticket, index) => (
          ...
          >
            ...
            <div
              className="w-[450px] flex justify-start truncate"
            ...
            <div className="w-[180px] flex justify-center">
              {ticket.fecha_envio
                ? new Date(ticket.fecha_envio).toLocaleDateString()
                : "Pendiente"}
            </div>
      ...
      {form && (
        <EnviarSolucion
          idSoporte={selectedId}
          close={() => setSupport(false)}
        />
      )}
